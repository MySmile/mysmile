...
class PagesManager(models.Manager):
    ...
    def get_content(self, request, lang=None, slug=None):
        try:
            ...
            slugs = Page.objects.filter(status=Page.STATUS_PUBLISHED, ptype=PTYPE_MENU).values_list('slug', flat=True).order_by('sortorder')
            menues = Page_translation.objects.filter(lang=lang, page__status=Page.STATUS_PUBLISHED, page__ptype=PTYPE_MENU).values_list('menu', flat=True).order_by('page__sortorder')
